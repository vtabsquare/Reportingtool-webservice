...
type Report = { id:string; name:string; published_at:string; updated_at?:string; pages:number; role:string; sourceType?:string };
...
export function ReportContentList({reports,onView,onSettings,onShare,onRefresh,onDelete}:{reports:Report[];onView:(r:Report)=>void;onSettings:(r:Report)=>void;onShare:(r:Report)=>void;onRefresh:(r:Report)=>void;onDelete:(r:Report)=>void}){
  ...
  return <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:8,boxShadow:"0 1px 2px rgba(0,0,0,.04)",overflow:"visible"}}>
    ...
    {reports.map(report=><div key={report.id} onDoubleClick={()=>onView(report)} style={{display:"grid",gridTemplateColumns:"minmax(260px,1fr) 130px 130px 150px 54px",padding:"11px 16px",borderBottom:"1px solid #eef0f2",alignItems:"center",fontSize:13,minHeight:42}}>
      ...
      <span style={{color:"#5f6368"}}>Report</span><span style={{color:"#5f6368"}}>{report.role}</span><span style={{color:"#5f6368"}}>{relative(report.updated_at||report.published_at)}</span>
      <div style={{position:"relative",justifySelf:"end"}} onClick={e=>e.stopPropagation()}><button aria-label={`More options for ${report.name}`} title="More options" onClick={e=>{e.stopPropagation();setMenu(menu===report.id?null:report.id)}} style={{width:34,height:30,border:"1px solid transparent",borderRadius:5,background:menu===report.id?"#eef2ff":"transparent",fontSize:22,lineHeight:1,cursor:"pointer",color:"#374151"}}>⋯</button>
      {menu===report.id&&<div style={{position:"absolute",right:0,top:34,zIndex:40,width:220,background:"#fff",border:"1px solid #d1d5db",borderRadius:7,boxShadow:"0 8px 24px rgba(0,0,0,.18)",padding:5}}>
        <Menu label="Open report" icon="▥" onClick={()=>onView(report)}/>
        {canEdit(report.role)&&<><Menu label="Share and manage access" icon="♧" onClick={()=>onShare(report)}/><Menu label="Refresh now" icon="↻" onClick={()=>onRefresh(report)}/><div style={{height:1,background:"#e5e7eb",margin:"5px 0"}}/><Menu label="Settings" icon="⚙" onClick={()=>onSettings(report)}/><Menu danger label="Delete" icon="⌫" onClick={()=>onDelete(report)}/></>}
      </div>}</div>
    </div>)}
  </div>
}
...
export default function ReportServiceSettings({report,onClose,onShare,onRefresh,onDelete}:Props){
  ...
  const sections=[['general','General','ⓘ'],['access','Sharing and access','♧'],['connections','Gateway and cloud connections','⌁'],['credentials','Data source credentials','🔑'],['refresh','Scheduled refresh','↻'],['history','Refresh history','◷']];
  return <div style={{position:"fixed",inset:0,zIndex:1250,background:"rgba(15,23,42,.28)"}} onMouseDown={onClose}>
    <aside style={{position:"absolute",right:0,top:0,bottom:0,width:"min(760px,92vw)",background:"#fff",boxShadow:"-10px 0 35px rgba(15,23,42,.2)",display:"flex",flexDirection:"column"}} onMouseDown={e=>e.stopPropagation()}>
      <header style={{height:58,padding:"0 20px",display:"flex",alignItems:"center",borderBottom:"1px solid #e5e7eb"}}><div style={{flex:1}}><b style={{display:"block",fontSize:16,color:"#202124"}}>Settings for {report.name}</b><small style={{color:"#6b7280"}}>Semantic model and report</small></div><button onClick={onClose} style={{border:0,background:"transparent",fontSize:22,cursor:"pointer"}}>×</button></header>
      <div style={{display:"flex",minHeight:0,flex:1}}><nav style={{width:245,borderRight:"1px solid #e5e7eb",padding:"12px 8px",overflow:"auto"}}>{sections.map(item=><button key={item[0]} onClick={()=>setSection(item[0])} style={{width:"100%",border:0,borderLeft:section===item[0]?"3px solid #7c3aed":"3px solid transparent",background:section===item[0]?"#f5f3ff":"transparent",padding:"10px 12px",display:"flex",gap:10,textAlign:"left",cursor:"pointer",fontSize:13,color:section===item[0]?"#5b21b6":"#374151",fontWeight:section===item[0]?700:500}}><span>{item[2]}</span>{item[1]}</button>)}</nav>
        <main style={{padding:26,overflow:"auto",flex:1,color:"#202124"}}>
          {section==="general"&&<Section title="General" text="Review the identity and ownership of this published item."><Read label="Name" value={report.name}/><Read label="Item type" value="Report and semantic model"/><Read label="Report ID" value={report.id}/><Read label="Your access" value={report.role}/></Section>}
          ...
          {section==="connections"&&<Section title="Gateway and cloud connections" text="Map this semantic model to a cloud connection. Local and private-network sources will use VTAB Gateway when installed."><Action title="Connection mapping" detail="Choose PostgreSQL, SQL Server, MySQL, MariaDB or Google Sheets and map every model table." button="Configure connection" disabled={!editable} onClick={onRefresh}/></Section>}
          {section==="credentials"&&<Section title="Data source credentials" text="Credentials are encrypted by the Services API and are never returned to the browser."><Action title="Credentials and authentication" detail="Test and securely save the connection used by this semantic model." button="Edit credentials" disabled={!editable} onClick={onRefresh}/></Section>}
          {section==="refresh"&&<Section title="Scheduled refresh" text="Configure when imported data is refreshed and keep the last successful snapshot available if a run fails."><Action title="Refresh schedule" detail="Set frequency and timezone, pause or resume, or request an immediate refresh." button="Configure schedule" disabled={!editable} onClick={onRefresh}/></Section>}
          {section==="history"&&<Section title="Refresh history" text="Review status, trigger, duration, processed rows and error details for recent runs."><Action title="Run history" detail="Open Refresh Center to inspect every run and retry failures." button="View refresh history" disabled={!editable} onClick={onRefresh}/></Section>}
